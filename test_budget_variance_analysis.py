"""
Test Budget Variance Analysis SQL for Infrastructure Department 2024

This script:
1. Creates sample infrastructure department data for 2024
...
def create_sample_data():
    """Create sample infrastructure department data for 2024"""
    print("=" * 80)
    print("Creating Sample Infrastructure Department Data for 2024")
    print("=" * 80)
    ...
    with sqlite3.connect(str(db_path)) as conn:
        ...
        cursor.execute("SELECT COUNT(*) FROM departments WHERE department = 'Infrastructure' AND date LIKE '2024%'")
        existing_count = cursor.fetchone()[0]
        
        if existing_count > 0:
            print(f"Found {existing_count} existing records. Deleting...")
            cursor.execute("DELETE FROM departments WHERE department = 'Infrastructure' AND date LIKE '2024%'")
        ...
        for month in range(1, 13):
            ...
            for _ in range(num_records):
                ...
                # Generate realistic budget and actual values
                base_budget = random.uniform(50, 200)  # $50k - $200k
                variance_factor = random.uniform(-0.15, 0.20)  # -15% to +20% variance
        ...
        conn.commit()
        print(f"Created {len(records)} sample infrastructure department records for 2024")
        return len(records)
...
def test_queries():
    ...
    print("\n" + "=" * 80)
    print("Testing Budget Variance Analysis Queries")
    print("=" * 80)
...
def main():
    ...
    try:
        ...
        print("\n" + "=" * 80)
        print("Budget Variance Analysis Test Complete")
        print("=" * 80)
        print("\nSQL queries are saved in: budget_variance_analysis.sql")
